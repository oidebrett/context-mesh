...
# We run prisma generate, migrate deploy, and seed mappings in the runner to ensure the DB is ready
CMD ["sh", "-c", "npx prisma generate --schema back-end/prisma/schema.prisma && npx prisma migrate deploy --schema back-end/prisma/schema.prisma && cd back-end && npx tsx src/seed_mappings.ts && cd .. && npx concurrently -n back,front \"npm run start:prod -w back-end\" \"npm run start -w front-end\""]
